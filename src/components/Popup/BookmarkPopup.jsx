...
import { Check, Bookmark, Plus, ExternalLink, ChevronDown } from 'lucide-react';
import * as Icons from 'lucide-react';
import Button from '../UI/Button';

export default function BookmarkPopup() {
  const { state, addBookmark, saveStateNow } = useApp();
  ...
  useEffect(() => {
    ...
    if (typeof chrome !== 'undefined' && chrome.tabs) {
      chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        ...
        if (currentTab) {
          // Don't allow saving internal chrome pages or the new tab itself
  ...
  useEffect(() => {
    if (state.sections.length > 0 && !sectionId) {
  ...
  if (isSaved) {
    return (
      <div className="p-8 flex flex-col items-center justify-center space-y-4 bg-[#0a0a0a] text-white min-h-[250px]">
        ...
        <p className="text-gray-400 text-sm">Saved to {selectedSection?.name}</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 bg-[#0a0a0a] text-white min-h-[300px] flex flex-col">
      ...
      <div className="mt-8 flex gap-2">
        ...
        <Button variant="primary" onClick={handleSave} className="flex-1" icon={Plus}>
          Add Bookmark
        </Button>
      </div>
    </div>
